...
static const uint32_t cli_delay = 20;  // Delay in [ms]
enum {BUF_LEN = 10}; // Number of elements in sample buffer
enum {MSG_LEN = 100}; // Max charachters in message body
...
// Message struct to wrap string for queue
...
// Only ISR calls this at the moment, no need to make it thread-safe
void IRAM_ATTR swap() {
...
void IRAM_ATTR onTimer() {
  ...
  if (idx >= BUF_LEN) {
    ...
    // If reading is not done, set overrun flag. 
    // We don't need to set this as a critical section, as
    // nothing can interrupt and change either value
...
void calcAverage(void *parameter) {
  ...
  while(1) {
    ...
    // Calcualte average (as floating point value)
    ...
    // So, we send a message to be printed out to serial terminal
    ...
    // Clearing the overrun flag and giving the "done reading" semaphore must be
    // done together without being interrupted
...
void setup() {
  ...
  // We want the done reading semaphore ro initialize at 1
  xSemaphoreGive(sem_done_reading);

  // Create message queue before it is used
  msg_queue = xQueueCreate(MSG_QUEUE_LEN, sizeof(Message));

  // Start task to handle cli events. Set at a higher priority
  // but it only runs once every cli_delay[ms]
