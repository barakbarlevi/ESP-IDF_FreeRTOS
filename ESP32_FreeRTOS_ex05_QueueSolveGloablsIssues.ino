...
static const char command[] = "delay "; // Note the space char
static const int delay_queue_len = 5; // Size of delay queue
static const int msg_queue_len = 5; // Size of message queue
static const uint8_t blink_max = 100; // Num of times to blink before message
...
void doCLI(void *parameter) {
  ...
  // Loop forever
  while(1) {
    // See if there's a message in the queue (don't block)
    ...
    if (Serial.available() > 0) {
      ...
      // Print new line an dcheck input on 'enter'
      if ((c == '\n') || (c == '\r')) {

        // Print new line to terminal
        Serial.print("\r\n");

        // Check if the first 6 chars are "delay "
        if (memcmp(buf, command, cmd_len) == 0) {

          // Convert last part to positive integet (negative int crashes (?))
          ...
          if (xQueueSend(delay_queue, (void *)&led_delay, 10) != pdTRUE) {
            Serial.println("ERROR: Couln't put item on delay queue.");
          }
        }
      ...
      // Otherwise (first 6 chars aren't "delay "), echo to serial terminal
      } else {
        Serial.print(c);
      }
    }
...
// Task: flash LED based on delay provided, notify other task every blink_max blinks
void blinkLED(void *parameter) {
  ...
  while(1) {
    ...
    // If we've blinked blink_max times, send a message to other task and reset counter
    counter++;
...
void setup() {
  ...
  Serial.println();
  Serial.println("---FreeRTOS demo queue part 2---");
